...
&lt;p&gt;CIBL is a browsable geographic database for crowd-sourcing traffic violation reports. Originally designed to publicly track illegal automotive encroachment into New York City bike lanes at carsinbikelanes.nyc, CIBL can be adapted for to document any sort of observable traffic violations within a defined geographic area. CIBL records the time, date, cross streets, GPS coordinates, user description and image of each record submitted. CIBL\'s setup wizard should be able to self-deploy in a LAMP environment upon navigating to /index.php in a web browser.&lt;/p&gt;' );
	...
	"'use_google' => getenv('GOOGLE_ENABLE') || " . $new_config['use_google'] . ",\n" .
	"'google_api_key' => getenv('GOOGLE_API_KEY') || '" . $new_config['google_api_key'] . "',\n" .
	"'google_extra_layer' => '" . $new_config['google_extra_layer'] . "',\n" .
	"'use_bing' => getenv('BING_ENABLE') || " . $new_config['use_bing'] . ",\n" .
	"'bing_api_key' => getenv('BING_API_KEY') || '" . $new_config['bing_api_key'] . "',\n" .
	...
	if (getenv('CLEARDB_DATABASE_URL') || getenv('JAWSDB_URL')){
		$text +=
			"\$database_url_from_config = parse_url(getenv('CLEARDB_DATABASE_URL') || getenv('JAWSDB_URL'));" .
			"\$connection = mysqli_connect(\$database_url_from_config['host'],\$database_url_from_config['user'],\$database_url_from_config['pass'],substr(\$database_url_from_config['path'], 1));\n\n" .
	}else{
		$text +=
			"\$config['sqlhost'] = '" . $new_config['sqlhost'] . "';\n" .
			"\$config['sqluser'] = '" . $new_config['sqluser'] . "';\n" .
			"\$config['sqlpass'] = '" . $new_config['sqlpass'] . "';\n" .
			"\$config['database'] = '" . $new_config['database'] . "';\n" .
